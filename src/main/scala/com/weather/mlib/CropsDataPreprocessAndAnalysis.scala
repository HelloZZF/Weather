    ...
    def dataCorrelation(flag:Int, data:RDD[LabeledPoint], featureA:String, indexA:Int, featureB:String, indexB:Int, kind:Double = -1) = {
        ...
        if (flag == 1) {
            if (kind == 1.0) {
                ...
                println("相同种类适合范围的" + featureA + "和" + featureB + "的相关系数为：" + correlation)
                ...
                val correlation = Statistics.corr(featureRDDA,featureRDDB)
                println("相同种类不适合范围的" + featureA + "和" + featureB + "的相关系数为：" + correlation)
            }
        }else if (flag == 2){
            val featureRDDA = data.filter(f => {
                f.label == 1.0
            ...
            val correlation = Statistics.corr(featureRDDA,featureRDDB)
            println("不同种类" + featureA + "和" + featureB + "的相关系数为：" + correlation)
        }
    }
