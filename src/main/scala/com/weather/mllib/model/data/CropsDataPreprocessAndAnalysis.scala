package com.weather.mllib.model.data
...
/**
  ...
  * 常用的方法有数据的向量矩阵化、正则化特征、数据降维和特征值提取）和数据清洗
  * 3、数据分析
  * 主要包括计算数据的最小值最大值，计算整体的平均偏差和标准偏差，以及查看数据的分布
  * 其次，标准偏差和其他分发值可以提供有关结果的稳定性和准确性等有用的信息
  * 如果数据有很大的问题需要进行特征标准化（决策树和朴素贝叶斯不受特征标准化的影响）
  * 4、模型的调整
...
object CropsDataPreprocessAndAnalysis {
    def main(args: Array[String]): Unit = {
        ...
        val data = MLUtils.loadLibSVMFile(sc, "C:\\Users\\admin\\Desktop\\Weather\\WeatherData\\MLlib\\crops.txt")
        //去除重复项，各个特征之间相关系数的确定
